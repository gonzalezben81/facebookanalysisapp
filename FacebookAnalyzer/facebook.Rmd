...
table_two <- params$table
    ...
    # text(barplot_one,round(x = sort(colSums(prop.table(value[, 1:8]))),digits = 1))
...
# text(barplot_one, 1, round(x = sort(colSums(prop.table(value[, 9:10]))),digits = 2),cex=1,pos=3)
...
# sentimentscores <- as.data.frame(sentimentscores)
# qplot(sentimentscores, data = sentimentscores, geom = "bar", ylab = "# movies")
```
